...
class _SingleTabConnectionManagerDaemon(_SingleTabConnectionManager):
    ...
    def __init__(
        ...
        super().__init__(
            chrome=self.chrome, index=index, auto_close=auto_close, flatten=flatten
        )
...
class OffsetDragWalker(OffsetMoveWalker):
    ...
    def __init__(self, start_x, start_y, tab: AsyncTab, button="left", timeout=None):
        super().__init__(start_x, start_y, tab=tab, timeout=timeout)
        self.button = button

    async def start(self):
        await self.tab.mouse_press(
            self.start_x, self.start_y, button=self.button, timeout=self.timeout
        )
        while self.path:
            x, y, duration = self.path.pop(0)
            await self.tab.mouse_move_rel(
            ...
            try:
                ...
                if func:
                    ...
                    if asyncio.iscoroutinefunction(func):
                        result = await func(event=event, tab=self.tab, buffer=self)
                    else:
        ...
        while not self._shutdown:
            ...
            if timeout is None or timeout > 0:
                try:
                    ...
                    callback = self.event_callbacks.get(event_name, self.callback)
                    if callback:
...
class FetchBuffer(EventBuffer):
    ...
    def match_event(self, event: dict, RequestPattern: dict):
        ...
        if not url or fnmatchcase(url, urlPattern):
            ...
            if resourceType:
                resource_type = self.tab.get_data_value(
    ...
    async def continueRequest(
        ...
        **_kwargs,
        ...
        _kwargs["requestId"] = requestId
        for key, value in dict(
            url=url, method=method, postData=postData, headers=headers
        ).items():
            if value is not None:
                _kwargs[key] = value
...
class IncognitoTabContext:
    ...
    async def __aenter__(self) -> AsyncTab:
        ...
        self.connection = self.browser_context.new_tab(
            ...
            enableBeginFrameControl=self.target_kwargs["enableBeginFrameControl"],
